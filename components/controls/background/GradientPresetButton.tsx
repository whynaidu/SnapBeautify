...
import type { GradientPresetButtonProps } from './types';

export const GradientPresetButton = memo(function GradientPresetButton({
    gradient,
    isSelected,
    isPremium,
    onClick,
}: GradientPresetButtonProps) {
    return (
        <button
            onClick={onClick}
            className={cn(
                'relative w-full aspect-square rounded-lg transition-all group gradient-grid-item',
            ...
            aria-label={`Select gradient ${gradient.name}${isPremium ? ' (Pro feature)' : ''}${isSelected ? ', currently selected' : ''}`}
        >
            {isSelected && (
